...
public class SubmissionServiceImpl implements SubmissionService {
    ...
    public Submission submitAssignment(Long assignmentId, Long studentId, String content, MultipartFile file) {
        try {
            ...
            throw new BusinessException(ErrorCode.OPERATION_FAILED, "提交作业失败: " + e.getMessage());
        }
    }
    ...
    public Submission saveDraft(Long assignmentId, Long studentId, String content) {
        try {
            ...
            throw new BusinessException(ErrorCode.OPERATION_FAILED, "保存草稿失败: " + e.getMessage());
        }
    }
    ...
    public PageResult<Submission> getStudentSubmissions(Long studentId, Integer page, Integer size) {
        ...
        List<Submission> submissions = submissionMapper.selectByStudentIdWithPagination(studentId, offset, size);
        Integer total = submissionMapper.countByStudentId(studentId);
        
        return PageResult.of(submissions, page, size, total.longValue(), (total + size - 1) / size);
    }
    ...
    public PageResult<Submission> getAssignmentSubmissions(Long assignmentId, Integer page, Integer size) {
        ...
        int offset = (page - 1) * size;
        List<Submission> submissions = submissionMapper.selectByAssignmentIdWithPagination(assignmentId, offset, size);
        Integer total = submissionMapper.countByAssignmentId(assignmentId);
    ...
    public PageResult<Map<String, Object>> getStudentGrades(Long studentId, Integer page, Integer size) {
        ...
        int offset = (page - 1) * size;
        List<Map<String, Object>> grades = submissionMapper.selectStudentGradesWithPagination(studentId, offset, size);
        Integer total = submissionMapper.countStudentGrades(studentId);
    ...
    public boolean hasSubmitted(Long assignmentId, Long studentId) {
        ...
        Integer count = submissionMapper.countByAssignmentAndStudent(assignmentId, studentId);
