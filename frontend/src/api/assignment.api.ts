...
export const assignmentApi = {
  // CRUD operations
  getAssignments: (params: { page?: number; size?: number; sort?: string; courseId?: string; teacherId?: string; status?: string; keyword?: string }): Promise<PaginatedResponse<Assignment>> => {
    return api.get('/assignments', { params });
  },

  getAssignmentsByCourse: (courseId: string, params: { page?: number; size?: number; sort?: string; assignmentType?: string }): Promise<PaginatedResponse<Assignment>> => {
  ...
  createAssignment: (data: AssignmentCreationRequest): Promise<Assignment> => {
    const payload = { ...data, dueDate: normalizeDueDate(data.dueDate) };
    return api.post('/assignments', payload);
  },

  updateAssignment: (id: string, data: AssignmentUpdateRequest): Promise<Assignment> => {
    const payload = { ...data, dueDate: normalizeDueDate(data.dueDate) };
    return api.put(`/assignments/${id}`, payload);
  },

  // bind only lessonId without touching title/desc validations
  bindLesson: (id: string, lessonId: string): Promise<void> => {
  ...
  remindUnsubmitted: (id: string, message?: string): Promise<{ sent: number; failed: number }> => {
