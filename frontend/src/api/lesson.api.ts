import type { Lesson } from '@/types/lesson';
import { api } from './config';

export const lessonApi = {
  getLessonsByCourse(courseId: string): Promise<Lesson[]> {
    return api.get(`/lessons/course/${courseId}`);
  },

  // 课程整体进度（百分比，后端 BigDecimal 0-100）
  getCourseProgressPercent(courseId: string, studentId?: string): Promise<number> {
    const params: any = {}
    if (studentId) params.studentId = studentId
    return api.get(`/lessons/course/${courseId}/progress-percentage`, { params })
  },

  getLesson(lessonId: string): Promise<Lesson> {
    return api.get(`/lessons/${lessonId}`);
  },

  // 学生在某课程下所有课节的进度列表
  getStudentCourseProgressList(courseId: string, studentId?: string): Promise<any[]> {
    const params: any = {}
    if (studentId) params.studentId = studentId
    return api.get(`/lessons/course/${courseId}/student-progress`, { params })
  },

  // New: get lesson materials via association table (方案A)
  getLessonMaterials(lessonId: string): Promise<any[]> {
    return api.get(`/lessons/${lessonId}/materials`);
  },

  // Teacher actions
  createLesson(data: Partial<Lesson>): Promise<Lesson> {
    return api.post('/lessons', data);
  },

  updateLesson(lessonId: string, data: Partial<Lesson>): Promise<Lesson> {
    return api.put(`/lessons/${lessonId}`, data);
  },

  deleteLesson(lessonId: string): Promise<void> {
    return api.delete(`/lessons/${lessonId}`);
  },

  // 设置章节内容（视频URL与资料文件绑定）
  updateLessonContent(lessonId: string, payload: { videoUrl?: string; materialFileIds?: number[]; allowScrubbing?: boolean; allowSpeedChange?: boolean }): Promise<any> {
    return api.put(`/lessons/${lessonId}/content`, payload);
  },

  updateLessonOrder(lessonId: string, order: number): Promise<void> {
    return api.put(`/lessons/${lessonId}/order`, { order });
  },

  // Student actions
  completeLesson(lessonId: string): Promise<void> {
    return api.post(`/lessons/${lessonId}/complete`, {});
  },

  updateLessonProgress(lessonId: string, payload: { progress?: number; studyTime?: number; lastPosition?: number }): Promise<void> {
    return api.post(`/lessons/${lessonId}/progress`, payload);
  },

  addLessonNotes(lessonId: string, notes: string): Promise<void> {
    return api.post(`/lessons/${lessonId}/notes`, { notes });
  },

  rateLesson(lessonId: string, rating: number): Promise<void> {
    return api.post(`/lessons/${lessonId}/rating`, { rating });
  },

  getLessonNotes(lessonId: string): Promise<{ notes: string }> {
    return api.get(`/lessons/${lessonId}/notes`);
  }
};
