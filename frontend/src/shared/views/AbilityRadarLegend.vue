<template>
  <div class="w-full">
    ...
    <div>
      <div v-for="(dim, idx) in normalizedDimensions" :key="idx" :class="idx>0 ? 'mt-4' : ''">
        <div class="flex items-center gap-2">
          <span class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ dim.title }}</span>
          <span class="text-xs text-gray-500">{{ dim.code }}</span>
        </div>
