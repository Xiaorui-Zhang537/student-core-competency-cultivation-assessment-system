<template>
  <div
    ref="sliderRef"
    :class="cn('w-full h-full overflow-hidden', props.class)"
    :style="{
      position: 'relative',
  ...
  >
    ...
    <!-- First Content -->
    <div class="relative z-20 size-full overflow-hidden" :style="{ pointerEvents: isInteracting ? 'none' : 'auto' }">
      <Transition>
        <div
          v-show="true"
          :class="
    ...
    <Transition>
      <div
        v-show="true"
        :class="cn('absolute top-0 left-0 z-[19] rounded-2xl w-full h-full select-none', props.secondContentClass)"
...
function mouseLeaveHandler(): void {
  ...
  if (props.slideMode === 'drag') isDragging.value = false
...
function handleEnd(): void {
  if (props.slideMode === 'drag') {
    ...
    isInteracting.value = false
    emit('drag:end')
    if (props.autoplay && !isMouseOver.value) startAutoplay()
  }
}
...
function handleMouseDown(_: MouseEvent): void { handleStart() }
function handleMouseMove(e: MouseEvent): void { handleMove(e.clientX) }
function handleTouchStart(_: TouchEvent): void { if (!props.autoplay) handleStart() }
function handleTouchEnd(): void { if (!props.autoplay) handleEnd() }
function handleTouchMove(e: TouchEvent): void { if (!props.autoplay) handleMove(e.touches[0].clientX) }

onMounted(() => { startAutoplay() })
onUnmounted(() => { stopAutoplay() })

watch(() => props.initialSliderPercentage, (v) => { sliderXPercent.value = v })
watch(() => props.autoplay, (v) => { if (v && !isMouseOver.value && !isDragging.value) { startAutoplay() } else { stopAutoplay() } })
</script>
