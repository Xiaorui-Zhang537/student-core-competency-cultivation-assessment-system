<template>
  <div class="pointer-events-none fixed left-0 top-0 z-50 size-full">
    <canvas
      id="fluid"
      ref="canvasRef"
...
onMounted(() => {
  ...
  function drawDisplay(target: FBO | null) {
    ...
    if ((config.SHADING) && (displayMaterial.uniforms as any).texelSize) {
      gl.uniform2f((displayMaterial.uniforms as any).texelSize, 1 / width, 1 / height)
    }
    if ((displayMaterial.uniforms as any).uTexture) {
