...
- 选择器/过滤器/搜索器统一以“学生端-我的作业（AssignmentsView）”为视觉基线：
  - 下拉触发器默认 `size="sm"`，优先使用信息色 `tint="info"`（除非业务语义更强）。
  - 搜索输入默认 `size="sm"`，`tint` 按页面语义设置（默认 `info`）。
...
## 更新记录
- v0.2.4：统一选择器/过滤器/搜索器到“学生端-我的作业”样式基线；旧课程发现页搜索输入曾调整为 `size="sm"`；教师端“创建/编辑作业”“编辑课程”页原生下拉改为 `GlassPopoverSelect` 并使用 `tint="info"`。
- v0.2.3：
