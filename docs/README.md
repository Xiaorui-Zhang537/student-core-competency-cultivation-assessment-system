...
欢迎来到“学生核心能力培养评估智能体”。本手册面向完全新手：不需要预先了解项目或相关技术，按顺序阅读并动手实践，你将能在本地跑起来并逐步理解系统。
...
### 能力雷达统一与写入修复（2025-09-22）
- 新增能力维度 `code` 列（唯一索引 `uk_code`），回填四维编码并设非空；提供存储过程迁移脚本（无 IF NOT EXISTS）。
- 教师评分写入：assessmentType 统一为 `assignment`，四维 relatedId=assignmentId；成绩维度根据百分比换算 0–5 写入。
- 归一化口径统一：四维/成绩写入时 `score<=5` 记 `max_score=5`；后端聚合均转为百分制。
- 修复 `assessed_at` 未设置导致时间过滤丢失：新增写入即刻设置；提供历史回填 SQL。
- 学生端/教师端雷达图维度顺序与本地化统一，支持导出 CSV/PNG。
