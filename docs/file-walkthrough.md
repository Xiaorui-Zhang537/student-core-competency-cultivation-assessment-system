---
title: 逐文件导览
description: 用最小集合理解关键入口文件、调用链与常见坑
outline: [2, 3]
---
...
- 路径默认以仓库根为起点：后端在 `backend/`，前端在 `frontend/`。
- 先看“入口文件与请求链路”，再看具体业务域（课程/作业/评分/AI）。
- 遇到问题时：从页面 -> Store -> API -> Controller -> Service -> Mapper 逆向定位，效率最高。

## 后端关键文件

### 1) `backend/src/main/java/com/noncore/assessment/AssessmentApplication.java`
...
### 2) `backend/src/main/java/com/noncore/assessment/controller/AiController.java`
...
- 调用链：`/ai/chat` → `AiService.generateAnswer` → `LlmClient`（按 provider 路由）→ 记录消息 → 返回

### 3) `backend/src/main/java/com/noncore/assessment/service/impl/StudentAnalysisServiceImpl.java`
- 职责：学生维度能力分析与指标聚合（示例）
...
### 4) `backend/src/main/java/com/noncore/assessment/service/impl/CourseServiceImpl.java`
...
### 5) `backend/src/main/java/com/noncore/assessment/service/impl/DashboardServiceImpl.java`
- 职责：教学仪表盘指标汇总（完成率、分布、趋势）
- 上游：`DashboardController`
- 下游：多 Mapper 聚合
- 注意：查询窗口、时间粒度与缓存策略（当前为内存缓存，无 Redis 依赖）
- 调用链：Controller → `DashboardServiceImpl` → 多 Mapper → 聚合返回

### 6) `backend/src/main/java/com/noncore/assessment/service/llm/GeminiClient.java`
- 职责：对接 LLM（Google Gemini）生成回答
- 上游：`AiService`
- 下游：外部 HTTP API
- 注意：密钥从环境变量注入（见 `backend/src/main/resources/application.yml` 的 `ai.providers.google.*`）
- 调用链：`AiService.generateAnswer` → `LlmClient` → `GeminiClient` → Provider → 返回文本

### 7) `backend/src/main/java/com/noncore/assessment/util/JwtUtil.java`
- 职责：JWT 生成/解析/刷新、JTI 黑名单
- 上游：`JwtAuthenticationFilter`、`AuthController`
- 下游：`io.jsonwebtoken` 库
- 注意：时效、时区、刷新逻辑与黑名单一致性

### 8) `backend/src/main/java/com/noncore/assessment/exception/GlobalExceptionHandler.java`
- 职责：统一异常转标准响应体
- 上游：全局
- 下游：`ApiResponse`
...
- 职责：全局配置（端口、context-path、数据源、JWT、上传、日志、SpringDoc、AI、CORS 放行）
- 注意：
  - 可由环境变量覆盖（尤其是 `DB_*`、`JWT_SECRET`、`GOOGLE_API_KEY` 等）
  - 本项目已排除 Redis 自动装配，缓存为本地内存（`spring.cache.type=simple`）
...
### 3) `frontend/src/layouts/StudentLayout.vue`、`frontend/src/layouts/TeacherLayout.vue`
- 职责：页面骨架、导航、背景与主题
- 上游：路由渲染
- 下游：特定视图
- 注意：配合背景组件与 UI Store 控制开关

### 4) `frontend/src/composables/useNotificationStream.ts`
- 职责：通知流（轮询/流式）封装
- 上游：顶栏通知中心、页面级订阅
- 下游：通知 API／消息中心
- 注意：防抖/去重、错误重试与断网恢复
...
### 6) `frontend/src/features/teacher/*`、`frontend/src/features/student/*`
