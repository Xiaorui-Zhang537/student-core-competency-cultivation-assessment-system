...
- `ability_goals`
  ...
  - 联动：系统每天早上会预同步“即将到期 / 已逾期”提醒；通知中心读取时还会再按需兜底同步。行为摘要会附带目标达成率、逾期后恢复次数、目标关联分数变化，以及最近截止/最近动作/当前聚焦等派生信号
  - 跳转：目标提醒在通知详情页点击“查看详情”后，会直接跳到学生端 `/student/analysis` 并滚动到“能力目标”区块
- `learning_recommendations`
  - 个性化建议
  - 用途：承接能力分析后的行动建议；当前会在学生首次读取建议列表且无可用记录时，按 `student_abilities` 最新快照自动生成基础建议
